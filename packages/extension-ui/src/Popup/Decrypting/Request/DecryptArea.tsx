...
function DecryptArea ({ buttonText, className, error, isExternal, isFirst, setError, decryptId }: Props): JSX.Element {
  ...
  useEffect(() => {
    ...
    !isExternal && isSignLocked(decryptId)
  ...
  }, [isExternal, decryptId]);

  const _onDecrypt = useCallback(
    (): Promise<void> => {
      setIsBusy(true);

      return approveDecryptPassword(decryptId, savePass, password)
        .then((): void => {
          setIsBusy(false);
          onAction();
        })
    ...
    [onAction, password, savePass, setError, setIsBusy, decryptId]
  );

  const _onCancel = useCallback(
    (): Promise<void> => cancelDecryptRequest(decryptId)
      .then(() => onAction())
      .catch((error: Error) => console.error(error)),
    [onAction, decryptId]
  );
...
export default styled(DecryptArea)`
